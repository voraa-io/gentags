...
def main():
    ...
    # NOTE: Use semantic threshold (τ=0.35) as GOLD STANDARD, not keyword-based
    ...
    if phase3_gini_path.exists():
        ...
        # Use semantic threshold (gold standard) if available, else keyword-based
        if 'gentag_gini_sem_thr' in phase3_gini.columns:
            gentag_gini_mean = phase3_gini['gentag_gini_sem_thr'].mean()
            gentag_gini_kw = phase3_gini['gentag_gini_kw'].mean()
            print(f"  Loaded Phase 3 Gini (Gold τ=0.35): gentags={gentag_gini_mean:.3f}")
            print(f"  (Keyword-based for reference: {gentag_gini_kw:.3f})")
        else:
            gentag_gini_mean = phase3_gini['gentag_gini'].mean()
            print(f"  Loaded Phase 3 Gini: gentags={gentag_gini_mean:.3f}")
        embedding_gini_mean = phase3_gini['embedding_gini'].mean()
        print(f"  Embedding Gini: {embedding_gini_mean:.3f}")
    else:
        gentag_gini_mean = 0.553  # Gold standard (semantic threshold τ=0.35)
        embedding_gini_mean = 0.369
        print(f"  Using Phase 3 gold standard values: gentags={gentag_gini_mean:.3f}, embeddings={embedding_gini_mean:.3f}")
    ...
    # NOTE: Gini comparison between gentags (drift Gini) and baselines (representation Gini)
    # is INVALID - these are different metrics. The valid comparison is gentags vs embeddings.
    print("\n⚠️  METHODOLOGICAL NOTE:")
    print("   Gini comparison with baselines is INVALID (different metrics)")
    print("   - Gentag Gini (0.553) = DRIFT Gini (change between run1 and run2)")
    print("   - Baseline Gini (0.12) = REPRESENTATION Gini (single snapshot)")
    print("   - Classical baselines are DETERMINISTIC (no run-to-run variation)")
    print("   - Valid comparison: Gentags (0.553) vs Embeddings (0.369) = 1.50x advantage")
    ...
    if retention_margin > 0.03:
        ...
        print(f"\n📊 RETENTION: Baselines win ({best_baseline_name} +{-retention_margin:.4f})")
        print("   This is expected — baselines are optimized for lexical overlap")
        print("   → Valid claim: 1.50x localization advantage over EMBEDDINGS (not baselines)")
